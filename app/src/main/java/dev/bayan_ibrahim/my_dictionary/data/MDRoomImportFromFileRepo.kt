...
class MDRoomImportFromFileRepo(
    ...
    private val wordWithTagsAndRelatedWordsDao = db.getWordsWithTagAndRelatedWordsDao()
    ...
    private class FileProcessingScope(
        ...
        val tagNameMapper = mutableMapOf<String, Long>()
    }
    ...
    override suspend fun processFile(
        ...
        try {
            db.withTransaction {
                ...
                processTags(scope)
                freeTagsMemoryIfNotRequired(scope)
                processWords(scope)
            }
        } catch (e: Exception) {
    ...
    private suspend fun processSingleTag(
        ...
        val tagId = dbTag?.first ?: wordClassDao.insertWordClass(tagData.asEntity(null))
    ...
    private suspend fun processTags(scope: FileProcessingScope) {
        ...
        scope.outputSummaryActions.onStep(MDFileProcessingSummaryActionsStep.ParseAndSaveTags)

        val tagsReader = scope.fileReader.getReaderOfPart(MDFilePartType.Tag) as MDFilePartReader.Tag
        val tags = tagsReader.readFile().map { it.toTag() }
        if (tags.firstOrNull() == null) { // empty
            scope.outputSummaryActions.onWarning(MDFileProcessingSummaryStepWarning.BlankTags)
            return
        }

        val dbTags = tagDao.getAllTags().first().associate { it.label.lowercase() to it.tagId!! }.toMutableMap()
        scope.tagNameMapper.putAll(dbTags)
        tags.forEach { tag ->
            if (tag == null) {
                // TODO, facing invalid context tag
            } else {
                val tagId = dbTags[tag.label.lowercase()]?.also {
                    scope.outputSummaryActions.recognizeTag(tag.label, false)
                } ?: let {
                    scope.outputSummaryActions.recognizeTag(tag.label, true)
                    tagDao.insertTag(tag.asEntity(null))
                }
                scope.tagNameMapper[tag.label.lowercase()] = tagId
            }
        }
    }

    private suspend fun freeTagsMemoryIfNotRequired(scope: FileProcessingScope) {
        if (MDFilePartType.Word !in scope.allowedParts) { // no need for cache data of languages
            scope.tagNameMapper.clear()
        }
    }
    ...
    private suspend fun processWordPart(scope: FileProcessingScope, wordPart: MDFileWordPart) {
        ...
        val word = try {
            wordPart.toWord().validateWithDatabase(
                dbWord = dbWordSource,
                conflictStrategy = scope.existedWordStrategy,
                corruptedStrategy = scope.corruptedWordStrategy,
                getDBTag = { tagData ->
                    resolveTag(scope, tagData)
                },
            ...
            when (e) {
                ...
                MDPropertyCorruptionException.AbortEntry -> {
            ...
            when (e) {
                ...
                MDPropertyConflictException.AbortEntry -> {
    ...
    private suspend fun handleWordExtraTags(
        ...
        if (scope.extraTags.isNotEmpty()) {
            ...
            if (shouldAdd) {
                wordCrossTagsDao.insertWordCrossTagsList(
                    wordCrossTag = scope.extraTags.map {
                        WordCrossTagEntity(id = null, wordId = newId, tagId = it.id)
                    }
                )
            }
        }
    ...
    private suspend fun fetchDbWordSource(dbWordId: Long?): Word? {
        return dbWordId?.let {
            wordWithTagsAndRelatedWordsDao.getWordWithTagsAndRelatedWordsRelation(it)
        }?.asWordModel()
    }
     ...
     */
    private suspend fun resolveTag(
        scope: FileProcessingScope,
        providedTagData: ParentedTag,
    ): ParentedTag {
        return scope.tagNameMapper[providedTagData.label.lowercase()]?.let { id ->
            tagDao.getTag(id)?.asModel()?.also {
                scope.outputSummaryActions.recognizeTag(it.label, false)
            }
        } ?: createNewTag(scope, providedTagData)
    }
    ...
    private suspend fun createNewTag(
        ...
        val newTagId = tagDao.insertTag(providedTagData.asEntity(null))
        scope.tagNameMapper[providedTagData.label.lowercase()] = newTagId
        scope.outputSummaryActions.recognizeTag(providedTagData.label, true)
        return ParentedTag(
            tag = providedTagData.onCopy(id = newTagId),
            parentId = providedTagData.parentId
        )
    }
    ...
    private suspend fun resolveWordClass(
        ...
        return let {
            ...
            wordClassDao.getWordClass(id)?.asWordClassModel()
    ...
    private suspend fun createNewWordClass(
        ...
        val dbTagId = wordClassDao.insertWordClass(
            tag = providedWordClassData.asEntity(null)
        )
    ...
    private suspend fun handleWordEntity(scope: FileProcessingScope, word: Word): Long {
        ...
        if (word.tags.isNotEmpty()) {
            val tagsRelations = word.tags.map { WordCrossTagEntity(tagId = it.id, wordId = id) }
            wordCrossTagsDao.insertWordCrossTagsList(tagsRelations)
        }
