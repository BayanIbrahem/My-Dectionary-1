...
class MDRoomContextTagRepo(
    ...
    override suspend fun getContextTag(id: Long): ContextTag? = contextTagDao.getContextTag(id)?.asModel()
    override fun getContextTagsStream(
        includeMarkerTags: Boolean,
        includeNonMarkerTags: Boolean,
    ): Flow<List<ContextTag>> = contextTagDao.getTagsOfMarkerStateAndLanguage(
